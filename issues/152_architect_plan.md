...
**现状**:
- 检查时目录内只有 `152_stage`（当前 Issue 的状态文件）
- `0_stage` 和 `plan.json` 可能在 Issue 创建后已被清理，或问题描述为预期待处理项
- **需确认**: 若历史垃圾文件存在，需清理脚本
...
add_executable(ini_parser_test ${CMAKE_CURRENT_LIST_DIR}/ini_parser_test.cpp
    ${CMAKE_CURRENT_LIST_DIR}/ini_parser.cpp)
...
**方案**: 保持 `file_finder.cpp` 为独立 CLI 程序不变；将其核心功能提取为 `file_finder_lib.cpp/h`，然后：
1. 创建 `file_finder_lib.cpp/h` 包含 `match_pattern`, `format_size`, `find_files` 等接口
2. `file_finder.cpp` 改为调用 `file_finder_lib`
3. `file_finder_test.cpp` 改为测试 `file_finder_lib`
4. 在 CMakeLists.txt 注册 `file_finder_test`

或更轻量方案：直接创建 `file_finder_lib.cpp/h`，将现有 CLI 的辅助函数提取为库函数，`file_finder.cpp` 依赖它，`file_finder_test.cpp` 测试库函数。
...
rm -f .pipeline-state/plan.json .pipeline-state/0_stage
```
...
9. **T4.1**: 在 `pipeline-runner.sh` 开头添加清理逻辑（删除 `0_stage`、`plan.json` 等无效文件）
10. **T4.2**: 手动执行清理
...
| .pipeline-state/ 清理误删有效文件 | 中 | 先列出待删除文件，人工确认后再删除；只删除明确的无害文件（0_stage, plan.json） |
